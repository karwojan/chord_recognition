...
pierwszy skrypt przygotowujący dane --- \url{src/dataset_scripts/01-generate_index_of_chordlab.py} ---
...
podkatalogów (po jednym na album). W każdym katalogu z albumem znajdują się pliki \filetype{txt} z
...
chodzi o indeksowanie plików z oznaczeniami, to autorzy dostarczają plik \filetype{txt} z opisem
...
\filetype{lab} zostały umieszczone w katalogu \url{data/chordlab/rwc_pop}.
...
umieszczone zostały w module \url{src/annotation_parser}.
...
ten ma formę zestawu klas pythonowy (w pliku \url{src/annotation_parser/chord_model.py}) i bazuje
...
\url{src/annotation_parser/labfile_printer.py}.
...
znać jedynie jego \code{video\_id} (\url{http://youtube.com/watch?v=\emph{video_id}}). Następnie
można filtrować dostępne formaty i ostatecznie pobrać wybrany strumień na dysk. Przygotowana została
funkcja pomocnicza (\url{src/dataset_scripts/downloading.py}), która dla danego \code{video\_id},
...
kolumnie \code{purpose} w pliku \url{data/index.csv} (wartości \code{train\_fold\_0},
\code{train\_fold\_1}, itd.). Wpis w tej kolumnie jest jednocześnie znacznikiem, że dla danego pliku z
...
\code{song\_mutliplier}) przejście przez wszystkie utwory z danego zbioru. Dla
każdego utworu losowana jest jedna z $12$ tonacji (jeżeli zastosowano augmentacje) i zwracanych jest
kilka (parametr \code{item\_multiplier}) 10-sekundowych fragmentów, zaczynających się w losowych
...
% cache
Ostatnim zabiegiem, zastosowanym we wstępnym przetwarzaniu danych, jest wspomniany już mechanizm
