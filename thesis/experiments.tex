...
% ewaluacja
Po przejściu przez wszystkie epoki, na koniec pojedynczego treningu, wykonywana jest dogłębna
ocena modelu. W tym celu wczytywane są wagi, dające najlepsze wyniki na zbiorze walidacyjnym i
wykonywana jest procedura ewaluacji modelu, na każdym z trzech zbiorów danych. Szczegóły tej
...
\subsubsection{Procedura ewaluacji modelu}

Ewaluacja wytrenowanego modelu odbywa się na zadanym zbiorze danych. Jak wspomniano, na koniec
każdego treningu jest ona wykonywana trzykrotnie: dla zbioru treningowego, walidacyjnego i
testowego. Podstawą całej procedury jest wytworzenia predykcji dla każdego utworu ze zbioru. Na
podstawie tych predykcji, dla każdego utworu wyliczane są:
\begin{itemize}
    ...
    \item wartości miar klasyfikacyjnych, liczonych tak, że pojedyncza ramka jest traktowana jak
        ...
        \end{itemize}
\end{itemize}
Ciąg wystąpień akordów zapisywany jest w pliku \filetype{lab}, metryki CSR, dokładność, czułość i
precyzja w pliku \filetype{json} a macierz pomyłek jako obrazek w pliku \filetype{png}. Po
wytworzeniu predykcji i obliczeniu metryk dla każdego utworu, powyższe metryki (CSR, dokładność,
czułość precyzja i macierz pomyłek) wyliczane są również dla całego zbioru (CSR zamienia się na
WCSR) i również zapisywane w dwóch plikach \filetype{json} i \filetype{png}. Cała implementacja
procedury ewaluacji modelu znajduje się w pliku \url{src/training/evaluate.py}.
...
\subsection{Etap 1. Reprodukcja wyników z pracy referencyjnej}
...
% strony z wynikami: powiększ, wklej, pomniejsz
\newpage
...
\input{results/results_final-small-btc-reproduce}
% TODO: dalsze wyniki
